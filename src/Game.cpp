...
Game::Game(sf::Vector2f windowSize)
    : Menu(windowSize), tools(windowSize), statsDisplay(windowSize) {
  try {
...
*/
void Game::drawBuiltDefender(vector<Defender *> builtTowerList,
...
*/
void Game::simpleAttackerGenerator(vector<Attacker *> &attackers,
  ...
  if (waveStart) {
    ...
    if (waveGenTime >=
        static_cast<float>(randomTimeframe) * 8 / difficulty * 1.5f) {
    ...
    if (deltaGenTime >=
        static_cast<float>(randomTimeframe) / difficulty * 1.5f) {
...
int Game::run(RenderWindow &renderWindow) {
  ...
  textureOfObject1.loadFromFile(assetFolder + "bul1.png");
  ...
  sf::Vertex line[] = {sf::Vertex(sf::Vector2f(0, 0)),
                       sf::Vertex(sf::Vector2f(500, 500))};

  line->color = Color(static_cast<sf::Uint8>(256), static_cast<sf::Uint8>(256),
  ...
  while (!gameOver && renderWindow.isOpen()) {
    ...
    for (int i = 0; i < static_cast<int>(coinsList.size()); i++) {
      renderWindow.draw(coinsList[i]->getSp());
    }
...

    {
      ...
      for (int i = 0; i < 60; i++) {
        ...
        if (landSprit.getGlobalBounds().contains(
                static_cast<float>(mouseRepostion.x),
    ...
    // there.
    if (nSelected >= 0 && nSelected < 60 && nInvenselected < 3 &&
        nInvenselected > -1 && landList[nSelected]->getEnpty() == true) {
    ...
    if (builtDefenderList.size() > 0) {
      for (unsigned int i = 0; i < builtDefenderList.size(); i++) {
        if (clock->getElapsedTime().asMilliseconds() >= 2000) {
          cout << allBullets.size() << endl;
          if (clock->getElapsedTime().asMilliseconds() >= 2000) {

            if (builtDefenderList[i]->getCost() == 40) {
              auto *temp = new Bullet(textureOfObject,
                                      builtDefenderList[i]->getPosition());
              allBullets.push_back(temp);
            } else if (builtDefenderList[i]->getCost() == 80) {
              auto *temp = new Bullet(textureOfObject1,
                                      builtDefenderList[i]->getPosition(), 2);
              allBullets.push_back(temp);
            }
  ...
  for (int i = 0; i < static_cast<int>(allAttackers.size()); i++) {
    delete allAttackers[i];
  }

  for (int i = 0; i < static_cast<int>(allBullets.size()); i++) {
    delete allBullets[i];
  }
  for (int i = 0; i < static_cast<int>(builtDefenderList.size()); i++) {
...
*/
void Game::handleTools(vector<Defender *> &defenders, vector<Land *> &landList,
    ...
    if (mouseStartClick && sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
      ...
      for (int i = 0; i < static_cast<int>(landList.size()); i++) {
        ...
        if ((landList[i]->getSprite())
                .getGlobalBounds()
                .contains(float(mousePressPosition.x),
                          float(mousePressPosition.y))) {
