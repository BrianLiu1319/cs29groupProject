...
void SettingsMenu::draw(sf::RenderTarget& target, sf::RenderStates states) const {
    ...
    //ADDNEW make sure the commented part is not there
    /*
...
Constructor.
 ...
 to the window size.
*********************************/
//ADDNEW make sure there is currDifficultyLevel(0)
SettingsMenu::SettingsMenu(sf::Vector2f windowSize) : Menu(windowSize), currDifficultyLevel(0){
    ...
    sf::Vector2f centerButtonPos(windowSize.x / 2.0f - buttonSize.x / 2, windowSize.y / 2.0f - buttonSize.y / 2 + buttonSize.y);
    
    //Set Buttons
    creditsButton.setSize(buttonSize);
    creditsButton.setPosition(sf::Vector2f(centerButtonPos.x, centerButtonPos.y + buttonSize.y * 1.25f));
    ...
    //musicButton.setPosition(windowSize.x/2 - musicButton.getSize().x * 1.1f, centerButtonPos.y - musicButton.getSize().x * 1.4f);
    ...
    sfxButton.setPosition(windowSize.x/2 + buttonSize.x * 0.05f, centerButtonPos.y - buttonSize.y * 1.25f);
    ...
    nextButton.setPosition(windowSize.x/2 + buttonSize.y * 1.5f, centerButtonPos.y - buttonSize.y * 2.6f);
    nextButton.setOutlineThickness(buttonSize.y / 20.0f);
    prevButton.setSize(sf::Vector2f(buttonSize.y / 3.0f, buttonSize.y));
    prevButton.setPosition(windowSize.x/2 - buttonSize.y * 1.5f - prevButton.getSize().x, centerButtonPos.y - buttonSize.y * 2.6f);
    ...
    try {
        ...
        setMusicTexture(musicButtonPath, musicMuteButtonPath, musicButtonInvPath, musicMuteButtonInvPath);
    ...
    text.setPosition(currDifficulty.getPosition().x + text.getCharacterSize() * 3.3f, currDifficulty.getPosition().y - buttonSize.y / 3.0f);
...
void SettingsMenu::setCreditsTexture(std::string path, std::string pathInv) {
    if(!creditsTexture.loadFromFile(path)) {
        throw(std::string(path));
    }
    if(!creditsTextureInv.loadFromFile(pathInv)) {
...
void SettingsMenu::setHowtoTexture(std::string path, std::string pathInv) {
    if(!howtoTexture.loadFromFile(path)) {
        throw(std::string(path));
    }
    if(!howtoTextureInv.loadFromFile(pathInv)) {
        throw(std::string(pathInv));
    }
...
*********************************/
void SettingsMenu::setSfxTexture(std::string path, std::string pathInv, std::string pathMute, std::string pathMuteInv) {
    if(!sfxTexture.loadFromFile(path)) {
        throw(std::string(path));
    }
    if(!sfxMuteTexture.loadFromFile(pathInv)) {
        throw(std::string(pathInv));
    }
    if(!sfxTextureInv.loadFromFile(pathMute)) {
        throw(std::string(pathMute));
    }
...
*********************************/
void SettingsMenu::setMusicTexture(std::string path, std::string pathInv, std::string pathMute, std::string pathMuteInv) {
    if(!musicTexture.loadFromFile(path)) {
        throw(std::string(path));
    }
    if(!musicMuteTexture.loadFromFile(pathInv)) {
        throw(std::string(pathInv));
    }
    if(!musicTextureInv.loadFromFile(pathMute)) {
        throw(std::string(pathMute));
    }
...
void SettingsMenu::setNextTexture(std::string path, std::string pathInv) {
    if(!nextTexture.loadFromFile(path)) {
        throw(std::string(path));
    }
    if(!nextTextureInv.loadFromFile(pathInv)) {
        throw(std::string(pathInv));
    }
...
void SettingsMenu::setPrevTexture(std::string path, std::string pathInv) {
    if(!prevTexture.loadFromFile(path)) {
        throw(std::string(path));
    }
    if(!prevTextureInv.loadFromFile(pathInv)) {
        throw(std::string(pathInv));
    }
...
int SettingsMenu::run(sf::Vector2f mousePos) {
    ...
    if(handleSoundButton(musicButton, musicTexture, musicTextureInv, musicMuteTexture, musicMuteTextureInv, mousePos)) {
        state = 4;
    }
    
    //Handle actions on the sfx button.
    if(handleSoundButton(sfxButton, sfxTexture, sfxTextureInv, sfxMuteTexture, sfxMuteTextureInv, mousePos)) {
        state = 5;
    }
    
    //ADD01
    //Handle actions on the next and prev buttons.
    if(currDifficultyLevel < numOfDifficulty - 1 && handleButton(nextButton, nextTexture, nextTextureInv, mousePos)) {
        currDifficultyLevel++;
        currDifficulty.setTexture(&difficultyLevelTextures[currDifficultyLevel]);
        state = 6;
    }
    
    if(currDifficultyLevel > 0 && handleButton(prevButton, prevTexture, prevTextureInv, mousePos)) {
        currDifficultyLevel--;
        currDifficulty.setTexture(&difficultyLevelTextures[currDifficultyLevel]);
        state = 7;
    }
...
bool SettingsMenu::handleSoundButton(sf::RectangleShape& button, const sf::Texture& buttonTexture, const sf::Texture& buttonInvTexture, const sf::Texture& buttonMuteTexture, const sf::Texture& buttonMuteInvTexture, const sf::Vector2f& mousePos) {
