 ...
 *********************************/
void SettingsMenu::draw(sf::RenderTarget &target,
...
SettingsMenu::SettingsMenu(sf::Vector2f windowSize)
    : Menu(windowSize), currDifficultyLevel(0) {
  ...
  creditsButton.setPosition(sf::Vector2f(
  ...
  prevButton.setPosition(windowSize.x / 2 - buttonSize.y * 1.5f -
                             prevButton.getSize().x,
                         centerButtonPos.y - buttonSize.y * 2.6f);
  ...
  try {
    ...
    setSfxTexture(sfxButtonPath, sfxMuteButtonPath, sfxButtonInvPath,
                  sfxMuteButtonInvPath);
    setMusicTexture(musicButtonPath, musicMuteButtonPath, musicButtonInvPath,
  ...
  text.setPosition(currDifficulty.getPosition().x +
                       text.getCharacterSize() * 3.3f,
...
*********************************/
void SettingsMenu::setSfxTexture(std::string path, std::string pathInv,
                                 std::string pathMute,
...
*********************************/
void SettingsMenu::setMusicTexture(std::string path, std::string pathInv,
                                   std::string pathMute,
                                   std::string pathMuteInv) {
...
void SettingsMenu::setDifficultyTextures(std::string path) {
  for (int i = 0; i < numOfDifficulty; i++) {
    if (!difficultyLevelTextures[i].loadFromFile(path + std::to_string(i + 1) +
                                                 ".png")) {
      throw(std::string(path));
    }
  }
...
int SettingsMenu::run(sf::Vector2f mousePos) {
  ...
  if (handleButton(creditsButton, creditsTexture, creditsTextureInv,
                   mousePos)) {
    state = 2;
  }
  ...
  if (handleSoundButton(musicButton, musicTexture, musicTextureInv,
                        musicMuteTexture, musicMuteTextureInv, mousePos)) {
    state = 4;
  }

  // Handle actions on the sfx button.
  if (handleSoundButton(sfxButton, sfxTexture, sfxTextureInv, sfxMuteTexture,
                        sfxMuteTextureInv, mousePos)) {
    state = 5;
  }
