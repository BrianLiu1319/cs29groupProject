...
GameOver::GameOver(sf::Vector2f windowSize) : Menu(windowSize), score(0) {
	...
	try {
		...
		if (!clawMarkTexture.loadFromFile(clawPath)) { throw(clawPath); }
	...
	clawMark.setPosition(centerButtonPos.x, centerButtonPos.y - clawMark.getSize().y / 2.0f);
}
...
int GameOver::run(sf::Vector2f mousePos) {
	...
	if (!muteSfx && !sfxHasPlayed && gameOverSound.getStatus() != sf::Sound::Playing) {
		gameOverSound.play();
		sfxHasPlayed = true;
	}
