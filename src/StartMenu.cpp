...
*********************************/
StartMenu::StartMenu(sf::Vector2f windowSize) : mainMenu(windowSize), settings(windowSize), credits(windowSize), howToPlay(windowSize){
    gameState = mainM;
    status = 0;
    try {
...
void StartMenu::setBgSound(std::string path) {
    if(!bgMusicBuffer.loadFromFile(path)) {
