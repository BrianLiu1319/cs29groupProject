...
void ValueDisplay::draw(sf::RenderTarget& target, sf::RenderStates states) const {
    ...
    if(currentMode == countdown) target.draw(waveCountdown);
    else target.draw(currentWave);
}
...
ValueDisplay::ValueDisplay(sf::Vector2f windowSize) {
    ...
    try {
        if(!scoreDisplayTexture.loadFromFile(scoreDisplayPath)) {
            throw scoreDisplayPath;
        }
        scoreDisplay.setTexture(&scoreDisplayTexture);
        if(!moneyDisplayTexture.loadFromFile(moneyDisplayPath)) {
            throw moneyDisplayPath;
        }
...
void ValueDisplay::updateScore(int s) {
    scoreText.setString(std::to_string(s));
}

void ValueDisplay::updateMoney(int m) {
    moneyText.setString(std::to_string(m));
}

void ValueDisplay::setCountdownMode() {
    currentMode = countdown;
}

void ValueDisplay::setWaveMode() {
    currentMode = wave;
}
...
void ValueDisplay::updateWave(int w) {
    currentWave.setString("Wave: " + std::to_string(w));
}
