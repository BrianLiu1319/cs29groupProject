 ...
 *********************************/
void HighScoreEntry::draw(sf::RenderTarget &target,
...
HighScoreEntry::HighScoreEntry(sf::Vector2f windowSize)
  ...
  textBox.setPosition(centerButtonPos.x,
                      centerButtonPos.y + buttonSize.y * 0.5f);
...
Sets the texture for the confirm button.
 Throws the path of file if not found.
*********************************/
void HighScoreEntry::setConfirmButtonTexture(std::string path,
                                             std::string pathInv) {
...
int HighScoreEntry::run(sf::Vector2f mousePos, float deltaTime) {
  ...
  */
  ...
  if (!muteSfx && !congratsHasPlayed &&
      congrats.getStatus() != sf::Sound::Playing) {
...
void HighScoreEntry::setCongratsSfx() {
  if (!congratsSoundBuffer.loadFromFile(congratsPath)) {
    throw(std::string(congratsPath));
  }
