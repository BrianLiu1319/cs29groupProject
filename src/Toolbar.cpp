...
Toolbar::Toolbar(sf::Vector2f windowSize) : Menu(windowSize) {
    ...
    whistleButton.setPosition(windowSize.x - buttonSize.x * 1.2f, windowSize.y - buttonSize.y * 1.2f);
    healthButton.setPosition(windowSize.x - buttonSize.x * 1.2f * 2.0f, windowSize.y - buttonSize.y * 1.2f);
...
void Toolbar::setWhistleTexture() {
    if(!whistleTexture.loadFromFile(whistlePath)) {
        throw(std::string(whistlePath));
    }
    if(!whistleTextureInv.loadFromFile(whistlePathInv)) {
        throw(std::string(whistlePathInv));
    }
...
void Toolbar::setHealthTexture() {
    if(!healthTexture.loadFromFile(healthPath)) {
        throw(std::string(healthPath));
    }
    if(!healthTextureInv.loadFromFile(healthPathInv)) {
        throw(std::string(healthPathInv));
    }
...
**************/
bool Toolbar::handleButton(sf::RectangleShape& button, const sf::Texture& buttonTexture, const sf::Texture& buttonInvTexture, const sf::Vector2f& mousePos) {
