...
Toolbar::Toolbar(sf::Vector2f windowSize) : Menu(windowSize) {
  ...
  sf::Vector2f buttonSize(windowSize.y / 1080.0f * 130.0f,
                          windowSize.y / 1080.0f * 130.0f);
...
bool Toolbar::whistleSelected() {
  if (currentSelection == SELECTION::whistle) {
    return true;
  }
  return false;
}
...
bool Toolbar::healthSelected() {
  if (currentSelection == SELECTION::health) {
    return true;
  }
  return false;
}
...
int Toolbar::run(sf::Vector2f mousePos) {
  ...
  if (handleButton(whistleButton, whistleTexture, whistleTextureInv,
                   mousePos)) {
    state = 1;
    currentSelection = SELECTION::whistle;
    whistleButton.setOutlineColor(sf::Color::White);
    healthButton.setOutlineColor(sf::Color::Transparent);
  } else if (handleButton(healthButton, healthTexture, healthTextureInv,
                          mousePos)) {
    state = 2;
