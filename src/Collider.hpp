...
static const string dogeTower = assetFolder + "dogSprites3.png";
...
public:
    //ADDNEW2 changed scales to 0.3, 0.3 for the new bullet
    //ADDNEW3 add sfx
    Bullet() : Collider(bulSpritePath, RIGHT, { 25, 360 }, 25) { scale(0.3, 0.3); setSfx(); }
	Bullet(sf::Vector2f a) : Collider(bulSpritePath, RIGHT, a, 50) { scale(0.3, 0.3); setSfx(); }
	Bullet(const sf::Texture& temp, sf::Vector2f a) : Collider(temp, RIGHT, a) { scale(0.3, 0.3); setSfx(); }
