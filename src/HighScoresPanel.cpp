 ...
 *********************************/
void HighScoresPanel::draw(sf::RenderTarget &target,
...
HighScoresPanel::HighScoresPanel(sf::Vector2f windowSize) {
  ...
  scoresText.setCharacterSize(
      static_cast<unsigned int>(panel.getSize().y / 18));
  ...
  try {
    ...
    std::cout << "Unable to open existing high scores file " << fileName
              << std::endl;
...
void HighScoresPanel::writeHighScoresFile() {
  ...
  if (!fout) {
    std::cout << "Unable to open output file " << highScoreFilePath
              << std::endl;
    exit(1);
  }
