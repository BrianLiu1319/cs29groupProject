...
void Menu::setClickSound(std::string path) {
    if(!clickBuffer.loadFromFile(path)) {
        throw(std::string(path));
    }
...
bool Menu::handleButton(sf::RectangleShape& button, const sf::Texture& buttonTexture, const sf::Texture& buttonInvTexture, const sf::Vector2f& mousePos) {
