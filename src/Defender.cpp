...
Defender::Defender(sf::Vector2f a)
    : Collider(dogeTower1, DIR::DIRECTION::RIGHT, {a.x, a.y + 47.0f}, 100,
               0.0f),
  ...
  // ADDNEW3 for sfx
  try {
    if (!barkSfxBuffer.loadFromFile(barkFile)) {
      throw barkFile;
    }
...
Defender::Defender(sf::Vector2f a, float b)
    : Collider(dogeTower2, DIR::DIRECTION::RIGHT, {a.x, a.y + 47.0f}, 660,
               0.0f),
...
Defender::Defender(sf::Vector2f a, int b)
    : Collider(dogeTower, DIR::DIRECTION::RIGHT, {a.x, a.y + 47.0f}, 200, 0.0f),
...
bool Defender::isDefeated() const {
  if (currState == STATES::defeated && animator.isLastFrame()) {
    return true;
  }
  return false;
}
...
Sets the Defender to be defeated.
 ...
 it from looping.
**************/
void Defender::setDefeated() { // in main add check for dying to prevent looping
                               // to setDefeeated?
