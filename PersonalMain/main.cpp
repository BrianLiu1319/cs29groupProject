...
int main()
{
	...
	while (renderWindow.isOpen())
	{
		...
		*/
		...
		if (builtTowerList.size() > 0)  // If the number of towers that have been built is not zero, we draw the towers that have been built
		{
			drawBuiltTower(builtTowerList);
			for (int i = 0; i < builtTowerList.size(); i++)
			{
				pTowerHolder = builtTowerList[i];
				if (clock->getElapsedTime().asMilliseconds() >= 1000)
					fire(clock, pTowerHolder);						//fire, draw bullet list
			}
			if (clock->getElapsedTime().asMilliseconds() >= 1000)
			{
				clock->restart();
